// Generated on 2017-02-25 using generator-jhipster 4.0.2
'use strict';
...
gulp.task('ngconstant:dev', function () {
    return ngConstant({
        ...
        constants: {
            ...
            API_URL: "http://118.187.50.42:8778/pcms/",
            UAA_URL: "http://118.187.50.42:8778/uaa/",
            POP_URL: "http://118.187.50.42:8778/popservice/"
        },
...
gulp.task('sass', function () {
    sass(config.app + 'content/css/scss/**/*')
...
gulp.task('shareSass', function () {
    sass(config.app + 'app/share/css/scss/**/*')
        .on('error', function (err) {
            console.error('Error!', err.message);
        })
        .pipe(sourcemaps.init())
        .pipe(sourcemaps.write('./'))
        .pipe(gulp.dest(config.app + 'app/share/css/'))
});

gulp.task('share',  function() {
    return gulp.src(config.app + 'app/share/**/*')
        .pipe(gulp.dest('../personal-center/share'))
});
...
gulp.task('watch', function () {
    ...
    gulp.watch(config.app + 'content/css/scss/**/*', ['sass']);
    //同时开启两个项目时需要同时注销sass任务 否侧出现死循环
    // gulp.watch(config.app + 'app/share/css/scss/**/*', ['shareSass']);
    gulp.watch(config.app + 'app/share/**/*', ['share']);
